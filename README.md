...
**슬롯 전체가 아니라 슬롯 안의 특정 단어 하나만 배제하기**: `excludeSlots` 대신(또는 함께)
`excludeWords`를 쓰면 됩니다.

```json
{
  "id": "numsuffix-hwa-personname-only",
  "when": { "keywordMatches": "personName", "negate": true },
  "excludeWords": { "NUMSUFFIX": ["화"] }
}
```

- `when.negate: true`를 추가하면 조건이 반전됩니다 — 위 예시는 "personName에 매치되지
  **않을** 때"(즉 인명이 아닐 때) 적용되는 규칙입니다.
- `excludeWords`는 `{ 슬롯이름: [단어, 단어, ...] }` 형태로, 그 슬롯의 다른 단어는 그대로 두고
  나열된 단어만 후보에서 뺍니다.
- **한계**: `personName`처럼 순수 문자열 패턴 휴리스틱은 "인명이냐 사물이냐 이슈냐"를 진짜로
  구분하지 못합니다. 예를 들어 "화"를 "인명 키워드에서만 등장"하도록 이 규칙으로 막아도,
  "국밥"처럼 2~4자 한글 단독 일반명사는 `personName`에 똑같이 매치되어 여전히 통과하고,
  반대로 "한동훈 탄핵"처럼 공백이 섞인 정당한 이슈성 키워드는 매치되지 않아 오히려 걸러집니다.
  규칙만으로 완전히 못 막는 경우를 대비해, 문제가 되는 단어의 `weight`를 같은 슬롯의 다른
  단어보다 낮게 둬서(예: NUMSUFFIX 안에서 "화"만 6000, 나머지는 10000~32000) 노출 빈도라도
  줄이는 절충을 함께 쓰는 걸 권장합니다.
